    ...
    recurrence:
      event.isRecurring && event.recurrencePattern
        ? {
            ...
            endType: event.recurrenceEndDate
              ? "date"
...
function formDataToScheduledPayload(data: EventFormData): any {
  ...
  const endTime = new Date(
    `${data.end_date}T${data.end_time || "23:59"}`,
  ).toISOString();
  const durationMs =
...
export function ScheduleEventDialog({
  ...
  onSave: (
    event: Omit<
      ScheduledEvent,
      "id" | "createdAt" | "updatedAt" | "invitations"
    >,
...
function EventCard({
  ...
  const acceptedCount = event.invitations.filter(
    (i) => i.status === "accepted",
  ).length;
  const pendingCount = event.invitations.filter(
    (i) => i.status === "pending",
  ).length;
                  ...
                  {event.invitations.slice(0, 3).map((inv) => (
                    ...
                    >
                      <AvatarFallback className="text-[10px]">
                        {getInitials(
                          inv.inviteeName || inv.inviteeEmail.split("@")[0],
                        )}
                      </AvatarFallback>
                    </Avatar>
                  ))}
...
export function ScheduleTab() {
  ...
  const [editingHubEvent, setEditingHubEvent] = useState<ScheduledEvent | null>(
    null,
  );
  ...
  const editingCalEvent = editingHubEvent
    ? scheduledToCalendar(editingHubEvent)
    : null;
  ...
  const pastEvents = useMemo(() => {
    ...
    return allEvents
      ...
      .filter(
        (e) =>
          !(e.status === "scheduled" && new Date(e.startTime) >= new Date()),
      )
      .filter((e) => {
        const start = new Date(e.startTime).getTime();
        return start >= cutoff;
  ...
  const pendingCount = myInvitations.filter(
    (i) => i.status === "pending",
  ).length;
